...
static void EvaluateApplicants(RuleEngine<UnderwritingInput> engine, UnderwritingInput[] applicants)
{
    foreach (var applicant in applicants)
    {
        ...
        // Show rule execution details with severity
        if (engine.ExecutionLogs.Any())
        {
            Console.WriteLine("  Rule Execution Details:");
            foreach (var log in engine.ExecutionLogs)
            {
                var statusIcon = log.Passed ? "PASSED" : "FAILED";
                var severityText = log.Severity == RuleSeverity.Critical ? " [CRITICAL]" : " [SOFT]";
                Console.WriteLine($"    {statusIcon} {log.RuleName}{severityText} - {log.Message}");
            }
        }
